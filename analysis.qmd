...
ggplot(ranked_scorers, aes(x = fct_reorder(scorer, total_goals), y = total_goals)) +
  ...
  geom_text(aes(label = total_goals), hjust = -0.15, size = 3.2, color = "#2C3E50") +
  coord_flip() +
...
ggplot(longevity, aes(x = fct_reorder(scorer, years_active), y = years_active)) +
  ...
  geom_text(aes(label = years_active), hjust = -0.15, size = 3.2, color = "#1F77B4") +
  coord_flip() +
  theme_minimal(base_size = 13) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
...
ggplot(style_summary, aes(x = fct_reorder(scorer, open_play_share), y = open_play_share)) +
  ...
  geom_text(aes(label = paste0(open_play_share, "%")), hjust = -0.15, size = 3.2, color = "#E74C3C") +
  coord_flip() +
  theme_minimal(base_size = 13) +
